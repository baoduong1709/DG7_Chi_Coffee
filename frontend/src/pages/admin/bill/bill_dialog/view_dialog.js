...
import { DialogActions, DialogContent, DialogTitle, TextField, Button, Grid, Typography, TableContainer, Table, TableHead, TableRow, TableCell, TableBody } from '@mui/material';
    ...
    return (
        <React.Fragment>
            <StyledDialog open={isDialogOpened} onClose={handleCloseDialog} fullWidth maxWidth={"sm"}>
                <DialogTitle>{item.customer_name} - {item._id}</DialogTitle>
                <DialogContent sx={{display: "flex", flexDirection: "column"}}>
                    ...
                    >
                        <Grid item container direction="row">
                            ...
                            <Typography variant="subtitle1" marginLeft={2}>{item.customer_name}</Typography>
                        </Grid>
                        <Grid item container direction="row">
                            <Typography variant="subtitle1">{"Thời gian đến:  "}</Typography>
                            <Typography variant="subtitle1" marginLeft={3.5}>{dayjs(item.createdAt).format("HH:mm:ss DD/MM/YYYY").toString()}</Typography>
                        </Grid>
                        <Grid item container direction="row">
                            <Typography variant="subtitle1">{"Xác nhận lúc:  "}</Typography>
                            <Typography variant="subtitle1" marginLeft={4.2}>{item.updatedAt? dayjs(item.updatedAt).format("HH:mm:ss DD/MM/YYYY").toString(): ""}</Typography>
                        </Grid>
                        {item.table_id? 
                        <Grid item container direction="row">
                            <Typography variant="subtitle1">{"Bàn:  "}</Typography>
                            <Typography variant="subtitle1" marginLeft={11.5}>{item.table_id}</Typography>
                        </Grid>
                        : <Grid item container direction="row">
                            <Typography variant="subtitle1">{"Địa chỉ giao:  "}</Typography>
                            <Typography variant="subtitle1" marginLeft={5}>{customer.address}</Typography>
                        </Grid>}
                        <Grid item container direction="row">
                            <TableContainer>
                        ...
                        <Grid item container direction="row" justifyContent={"flex-end"} marginTop={3} alignItems={"center"}>
                            <Typography variant="subtitle1">{"Tổng tiền:  "}</Typography>
                            <Typography variant="subtitle1" marginLeft={2}>{item.cost}</Typography>
                        </Grid>
                    </Grid>
                </DialogContent>
                <DialogActions>
