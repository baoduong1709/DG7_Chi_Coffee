    ...
    return (
        ...
        >
            ...
            <Grid container direction="row" justifyContent="flex-end" alignItems="center">
                <Button variant="outlined" size="small" onClick={(e) => handleSubtractItem(e)}>
                    -
                </Button>
                <Typography variant="subtitle1" marginX={1}>
                    {item.quantity}
                </Typography>
                <Button variant="outlined" size="small" onClick={(e) => handleAddItem(e)}>
                    +
                </Button>
            </Grid>
        </Card>
                ...
                <Box>
                    <Grid container direction="row" columns={3} spacing={2}>
                        {isLoading ? (
                            ...
                            items
                                .filter((item) => item.product_status === true)
                                .map((item) => {
                                    return (
        ...
        <Box m="1.5rem 2.5rem" width="95%">
            <Box sx={{ marginTop: 2, display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
                ...
                <Box display={'flex'} flexDirection={'row'}>
                    <Button
                        variant={table === '' ? 'outlined' : 'contained'}
                        sx={{ marginX: 2 }}
                        onClick={() => setDialogOpen(true)}
                        color={'secondary'}
                    >
                        {table === '' ? 'Chọn bàn' : table}
                    </Button>
                    <Box sx={{ width: 250 }}>
                        <Autocomplete
                            id="free-solo-demo"
            ...
            <Box mt="40px" height="75vh" display={'flex'} flexDirection={'row'}>
                <Paper sx={{ width: '70%', marginRight: 2, overflow: 'auto' }}>
                    ...
                    {tabs?.map((child) => {
                        return <CustomTabPanel key={child.id} value={value} index={child.index} id={child.id} />;
                    })}
                </Paper>
                    ...
                    <Box height={'85%'} overflow={'auto'}>
                        <Grid container direction="column" justifyContent="space-around" alignItems="center">
                            ...
                            <Grid item sx={{ width: '100%' }}>
                                <Stack>
                                    {orderItems?.map((item) => {
                                        return <OrderedItem key={item._id} item={item} />;
                                    })}
                                </Stack>
                            </Grid>
                        </Grid>
                        ...
                        <Grid container direction="column" justifyContent="space-around" alignItems="center">
                            <Grid item>
                                <Box marginY={1} display={'flex'} flexDirection={'row'}>
                                    <Typography variant="subtitle1" fontSize={18} marginRight={1}>
                                        Tổng:
                                    </Typography>
                                    <Typography variant="subtitle1" fontSize={18} color={'primary'}>
                                        {formatter.format(getOrderTotal()).replace(/₫/g, 'VND')}
                                    </Typography>
                                </Box>
                            </Grid>
                            <Grid item>
                                <Box>
                                    ...
                                    <Button
                                        variant="contained"
                                        color="primary"
                                        onClick={(e) => handleOrderingConfirmed(e)}
                                    >
                                        Xác nhận
                                    </Button>
                                </Box>
            ...
            <TableDialog key={'table'} isDialogOpened={dialogOpen} handleCloseDialog={() => setDialogOpen(false)} />
            <ToastContainer />
        </Box>
    );
}
